...
if [ "$test3" != pass ]; then
	grep -Eq '^\s*net\.ipv6\.conf\.all\.disable_ipv6\s*=\s*1\b' /etc/sysctl.conf /etc/sysctl.d/*.com && grep -Eq '^\s*net\.ipv6\.conf\.default\.disable_ipv6\s*=\s*1\b' /etc/sysctl.conf /etc/sysctl.d/*.conf && sysctl net.ipv6.conf.all.disable_ipv6 | grep -Eq '^\s*net\.ipv6\.conf\.all\.disable_ipv6\s*=\s*1\b' && sysctl net.ipv6.conf.default.disable_ipv6 | grep -Eq '^\s*net\.ipv6\.conf\.default\.disable_ipv6\s*=\s*1\b' && test3="pass"
fi
